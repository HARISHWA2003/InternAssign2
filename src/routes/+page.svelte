<script lang="ts">
	...
	let searchString = '';
	$: selectedCar = cars.filter((cars) => {
		return cars.title.toLowerCase().includes(searchString.toLocaleLowerCase());
	});
	let nc = 6;
	export function showMore() {
		nc += 3;
	}
	$: specificCar = selectedCar.slice(0, nc);
</script>

<div class="relative">
	<div
		class="loc flex-1 flex flex-root bg-[#FAF9F6] h-[70px] mx-40 mt-4 rounded-lg items-center p-4"
	>
		<div class="flex-1 flex flex-row float-left">
			<div
				class="p-3 bg-[#D3D3D3] font-semibold w-[100px] rounded-3xl mr-4 flex items-center justify-center hover:bg-slate-200"
			>
				New York
			</div>
			<div
			...
			<div
				class="p-3 bg-[#6495ED] font-semibold w-[100px] rounded-3xl mr-4 flex items-center justify-center hover:bg-slate-200"
			>
				London
			</div>
		</div>
		<div
			class="p-3 border-2 font-semibold text-[#6495ED] border-[#6495ED] w-[150px] rounded-3xl mr-4 flex items-center justify-center hover:bg-slate-200 float-right"
		>
			View more
		</div>
	</div>
	<div class="flex flex-col w-full h-full justify-center items-center mt-2 px-24">
		<div class="cars grid grid-cols-3 gap-6 justify-between">
			{#each specificCar as car}
				<div
					class="car flex flex-col text-white justify-start border-2 bg-[#D3D3D3] w-[500px] h-[400px] p-4 rounded-xl border-neutral-400 hover:border-2 hover:border-neutral-600 shadow-md shadow-gray-400 hover:shadow-lg hover:shadow-gray-400 hover:-translate-y-1 transition cursor-pointer"
				>
					<div class="carimg bg-white rounded-md flex-1 mt-4">
						<img src="" alt="carimg" />
					</div>
					<div class="carname text-black font-bold text-lg mt-6">{car.title}</div>
					<div class="carclass text-black font-semibold text-lg">Class: {car.class}</div>
					<div class="carproduction text-black font-semibold text-lg">
						Year: {car.start_production}
					</div>
					<div class="flex flex-row items-center justify-between px-10">
						<div class="price text-black text-lg font-bold">Price: $400/month</div>
						<a
							href="/{car.id}"
							type="button"
							class="text-white mt-2 w-1/3 bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2"
							>Rent Now</a
						>
					</div>
				</div>
	...
	<div class="loc flex-1 flex h-[70px] mx-40 mt-4 rounded-lg items-center justify-center p-4">
		<button
			type="button"
			on:click={showMore}
			class="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br shadow-lg shadow-blue-500/50 dark:shadow-lg dark:shadow-blue-800/80 font-semibold rounded-lg text-sm px-5 py-5 text-center mr-2 mb-2"
			>Show more</button
		>
	</div>
</div>
